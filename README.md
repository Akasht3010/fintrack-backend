...
- Every money column (`Transaction.amount`, `Account.opening_balance`, `Budget.limit_amount`/`spent_amount`/`remaining_amount`) is `NUMERIC(12, 2)`, mapped to Python `Decimal` — not `Float` — so `SUM()`s and repeated balance updates can't drift the way binary floats do. The wire format is unaffected: response schemas use a `Money` type (`app/schemas/money.py`) that still serializes as a plain JSON number, so no frontend change was needed. `migrate_schema()` in `app/config/init_db.py` `ALTER`s an existing Postgres deploy's columns to `NUMERIC(12,2)` on boot; a fresh DB gets it straight from `create_all`.
- **Deploy:** one **Cloud Run** service (`Dockerfile`) serves both the API and the Expo web export (`app/web.py` mounts `./web`, the build the deploy step drops in), backed by **Cloud SQL** Postgres, with config in **Secret Manager**. A Cloud Build trigger on `main` builds + redeploys (`cloudbuild.yaml`). Live at `https://fintrack-989422306373.asia-south1.run.app`. Full details and the from-scratch runbook in **[DEPLOY_GCP.md](DEPLOY_GCP.md)**.
